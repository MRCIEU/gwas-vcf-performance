...
```{r}
Sys.setenv(n_sim=params$n_sim, n_variants=format(params$n_variants, scientific = FALSE))
```
...
```{bash}
echo "$n_variants"
for file in $(ls /data/*.vcf.gz | grep -v "sampled"); do
    echo "$file"
    # select header
    ...
    # index
    bcftools index -t $(echo "$file" | sed 's/\.vcf\.gz/_sampled\.vcf\.gz/g')
done
```
