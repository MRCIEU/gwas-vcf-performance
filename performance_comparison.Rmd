...
Extract single variant by rsid

```{bash}
snp="rs10518554"

# compressed text
time gzip -dc /data/gwas.txt.gz | awk -v snp="$snp" -F"\t" '$3==snp' > /dev/null

# plain text
gzip -dc /data/gwas.txt.gz > /data/gwas.txt
time awk -v snp="$snp" -F"\t" '$3==snp' /data/gwas.txt > /dev/null

# compressed VCF using bcftools
time bcftools query -i '%ID == "$snp"' -f '%LINE\n' /data/gwas.vcf.gz > /dev/null
time bcftools query -e '%ID != "$snp"' -f '%LINE\n' /data/gwas.vcf.gz > /dev/null

# compressed VCF using awk
time gzip -dc /data/gwas.vcf.gz | awk -v snp="$snp" -F"\t" '$3==snp' > /dev/null

# uncompressed VCF using awk
gzip -dc /data/gwas.vcf.gz > /data/gwas.vcf
time awk -v snp="$snp" -F"\t" '$3==snp' /data/gwas.vcf > /dev/null
```

Extract single variant by chromosome & position
...
# compressed text
time gzip -dc /data/gwas.txt.gz | awk -v chr="$chr" -v pos="$pos" -F"\t" '$1 == chr && $2 == pos' > /dev/null

# plain text
gzip -dc /data/gwas.txt.gz > /data/gwas.txt
time awk -v chr="$chr" -v pos="$pos" -F"\t" '$1 == chr && $2 == pos' /data/gwas.txt > /dev/null

# compressed VCF
time bcftools query -r "$chr:$pos-$pos" -f '%LINE\n' /data/gwas.vcf.gz > /dev/null
```

Extract interval with chromosome & position range
...
# compressed text
time gzip -dc /data/gwas.txt.gz | awk -v chr="$chr" -v start="$start" -v end="$end" -F"\t" '$1 == chr && $2 >= start && $3 <= end' > /dev/null
...
# compressed VCF
time bcftools query -r "$chr:$start-$end" -f '%LINE\n' /data/gwas.vcf.gz > /dev/null
```
